...
- **Known packaging quirk (unresolved)**: in manual testing, `Jarvis.exe` built with `console=True` reached `http://localhost:7860` (HTTP 200) in ~20s, matching the expected cold-start time. The same build with `console=False` (the shipped setting) sat idle — alive, CPU barely moving, no port bound, no window handle — for several minutes before being killed for diagnosis. Root cause not confirmed; candidates: Windows deprioritizing a process whose only window is created hidden and never shown (EcoQoS/background throttling), or contention from concurrent PyInstaller builds running on the test machine at the time. Needs a repeat test on an idle machine with nothing else running before treating this as a real defect vs. a one-off. If it reproduces cleanly, first thing to try: temporarily show the window instead of `hidden=True` and see if startup time returns to ~20s.
